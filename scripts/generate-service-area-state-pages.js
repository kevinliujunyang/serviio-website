...
function enPage(state) {
  ...
  const title = `${state.name} Chinese Restaurant AI Assistant`;
  const description = `Restaurant POS phone order integration and AI phone ordering for ${state.name} Chinese restaurants using 39 Miles, Square, Toast, Clover, MenuSifu, and Chowbus.`;
...
<head>
    ...
    <meta name="keywords" content="${state.name} Chinese restaurant AI assistant, ${state.name} restaurant AI phone answering, ${state.name} restaurant POS phone order integration, ${state.abbr} restaurant phone order AI">
    ...
    <script type="application/ld+json">{"@context":"https://schema.org","@type":"Service","name":${escapeJson(title)},"serviceType":"Restaurant AI assistant and AI phone ordering for Chinese restaurants","description":${escapeJson(description)},"provider":{"@type":"Organization","name":"Serviio","url":"${SITE_ORIGIN}"},"areaServed":{"@type":"State","name":${escapeJson(state.name)}},"audience":{"@type":"Audience","audienceType":"Chinese restaurant owners using restaurant POS systems"}}</script>
