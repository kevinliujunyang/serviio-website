...
const cities = [
  ...
  },
  {
    ...
    extraIntent: 'This page also supports restaurant AI assistant Boston searches from owners comparing AI answering, phone ordering, and POS-ready restaurant automation.',
    zhCity: '波士顿',
    zhState: '马萨诸塞州',
  ...
  },
  {
    ...
    extraIntent: 'It also helps owners compare restaurant POS phone order integration Philadelphia options for takeout-heavy Chinese restaurants.',
    zhCity: '费城',
    zhState: '宾州',
...
function enPage(city) {
  ...
  const title = `${city.city} Chinese Restaurant AI Assistant`;
  const description = `Restaurant POS phone order integration and AI phone ordering for ${city.city} Chinese restaurants using 39 Miles, Square, Toast, Clover, MenuSifu, and Chowbus.`;
  const stateHref = city.stateSlug ? `/service-areas/${city.stateSlug}/` : '/service-areas/';
  const extraIntent = city.extraIntent ? ` ${city.extraIntent}` : '';
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <script type="application/ld+json">{"@context":"https://schema.org","@type":"Service","name":${json(title)},"serviceType":"Restaurant AI assistant and AI phone ordering for Chinese restaurants","description":${json(description)},"provider":{"@type":"Organization","name":"Serviio","url":"${SITE_ORIGIN}"},"areaServed":{"@type":"City","name":${json(city.city)}},"audience":{"@type":"Audience","audienceType":"Chinese restaurant owners using restaurant POS systems"}}</script>
    ...
    <script type="application/ld+json">{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":${json(`Does Serviio serve as a restaurant AI assistant in ${city.city}?`)},"acceptedAnswer":{"@type":"Answer","text":${json(`Yes. Serviio can evaluate Chinese restaurants in ${city.city}, especially restaurants with steady phone orders and an existing POS system.`)}}},{"@type":"Question","name":"Which POS systems can be reviewed for restaurant POS phone order integration?","acceptedAnswer":{"@type":"Answer","text":"The lead form supports 39 Miles, Square, Toast, Clover, MenuSifu, Chowbus, Mealkeyway, other POS systems, and restaurants without a POS yet."}},{"@type":"Question","name":"What nearby markets are relevant?","acceptedAnswer":{"@type":"Answer","text":${json(`Relevant nearby markets include ${city.nearby}.`)}}}]}</script>
...
<body class="bg-white text-gray-900 antialiased">
    ...
    <main>
        ...
        <section id="contact" class="py-24 px-4 sm:px-6 lg:px-8 bg-gray-900"><div class="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center"><div><h2 class="text-4xl font-bold text-white mb-4">检查您的${city.zhCity}餐厅</h2><p class="text-xl text-gray-400 mb-8">提交城市、POS 系统和电话订单量，我们会确认 Serviio 是否适合您的${city.zhCity}餐厅。</p></div><div class="bg-white rounded-2xl p-8"><form id="contact-form" action="https://formspree.io/f/xeeezpzn" method="POST" class="space-y-6"><input type="hidden" name="_subject" value="${city.city} Service Area Lead - Serviio (中文)"><input type="hidden" name="_next" value="${url}#thank-you"><input type="hidden" name="lead_source" value="zh_service_area_${city.slug.replace(/-/g, '_')}"><input type="hidden" name="ideal_customer_profile" value="Chinese restaurant owner with existing POS or POS interest"><input type="hidden" name="conversion_offer" value="local_pos_fit_check"><input type="hidden" name="_language" value="zh"><div><label for="restaurant" class="block text-sm font-medium text-gray-700 mb-2">餐厅名称</label><input type="text" id="restaurant" name="restaurant" required class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" placeholder="金龙餐厅"></div><div><label for="name" class="block text-sm font-medium text-gray-700 mb-2">您的姓名</label><input type="text" id="name" name="name" required class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" placeholder="张先生"></div><div><label for="phone" class="block text-sm font-medium text-gray-700 mb-2">联系电话</label><input type="tel" id="phone" name="phone" required class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" placeholder="(555) 123-4567"></div><div><label for="email" class="block text-sm font-medium text-gray-700 mb-2">电子邮箱</label><input type="email" id="email" name="email" required class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" placeholder="you@restaurant.com"></div><div><label for="city" class="block text-sm font-medium text-gray-700 mb-2">城市</label><input type="text" id="city" name="restaurant_city" required value="${city.zhCity}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition"></div><div><label for="state" class="block text-sm font-medium text-gray-700 mb-2">州</label><input type="text" id="state" name="restaurant_state" required value="${city.abbr}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition"></div><div><label for="pos_system" class="block text-sm font-medium text-gray-700 mb-2">当前使用的 POS 系统</label><select id="pos_system" name="pos_system" required class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition"><option value="">请选择 POS</option><option>39 Miles</option><option>Square</option><option>Toast</option><option>Clover</option><option>MenuSifu</option><option>Chowbus</option><option>Mealkeyway</option><option>其他 POS</option><option>暂时没有 POS</option></select></div><div><label for="phone_orders" class="block text-sm font-medium text-gray-700 mb-2">每周大约电话订单量</label><select id="phone_orders" name="phone_orders_per_week" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" required><option value="">请选择范围</option><option>少于 25 单</option><option>25-75 单</option><option>76-150 单</option><option>150 单以上</option></select></div><div><label for="language_need" class="block text-sm font-medium text-gray-700 mb-2">需要接听的电话语言</label><select id="language_need" name="language_need" required class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition"><option value="">请选择语言</option><option>仅英文</option><option>普通话</option><option>粤语</option><option>普通话和粤语</option><option>英文和中文</option><option>西班牙语或其他语言</option></select></div><div><label for="main_pain" class="block text-sm font-medium text-gray-700 mb-2">主要电话订单痛点</label><select id="main_pain" name="main_pain" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" required><option value="">请选择主要痛点</option><option>高峰期漏接电话</option><option>普通话/粤语/英文双语电话</option><option>手动录入 POS</option><option>下班后电话</option><option>菜单问题或订单修改</option><option>其他</option></select></div><div><label for="pos_recommendation_interest" class="block text-sm font-medium text-gray-700 mb-2">如果还没有 POS，是否希望了解 POS 推荐？</label><select id="pos_recommendation_interest" name="pos_recommendation_interest" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" required><option value="">请选择</option><option>希望了解 POS 推荐</option><option>只需要 AI 电话接单</option><option>不适用，我已经有 POS</option></select></div><div><label class="flex gap-3 text-sm text-gray-700"><input type="checkbox" id="pos_partner_consent" name="pos_partner_consent" value="Yes, Serviio may share my request with POS providers or consultants" class="mt-1 h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"><span>如果我需要 POS 推荐，我同意 Serviio 可以把需求分享给 POS 服务商或顾问跟进。</span></label></div><div><label for="pos_purchase_timeline" class="block text-sm font-medium text-gray-700 mb-2">如果需要 POS 推荐，计划什么时候选择 POS？</label><select id="pos_purchase_timeline" name="pos_purchase_timeline" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition" required><option value="">选择时间</option><option>马上</option><option>1 个月内</option><option>1-3 个月</option><option>3-6 个月</option><option>还不确定</option><option>不适用，已经有 POS</option></select></div><button type="submit" class="w-full gradient-bg text-white py-4 rounded-lg font-semibold text-lg hover:opacity-90 transition">检查${city.zhCity}适配</button></form></div></div></section>
    </main>
...
for (const city of cities) {
  writeFile(`service-areas/${city.slug}/index.html`, addPosPurchaseTimeline(enPage(city), 'en'));
  writeFile(`zh/service-areas/${city.slug}/index.html`, addPosPurchaseTimeline(zhPage(city), 'zh'));
}
updateSitemap();
updateInternalLinks();
