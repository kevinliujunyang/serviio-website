# Serviio SEO Growth Plan

## Target

Rank Serviio for high-intent searches from Chinese restaurant owners in the United States who already use a POS system or are considering one.
...
- Restaurant phone answering service for takeout orders
- POS-integrated phone ordering
...
- Chinese equivalents under `/zh/pos/`.

## Deployment Checklist

1. Deploy the current static site to Cloudflare Pages.
   - Run `npm run build` and `npm run validate:seo` before pushing.
   - Use `docs/seo-deploy-and-lead-runbook.md` as the deploy and indexing checklist.
2. Confirm all URLs in `sitemap.xml` return HTTP 200 on production.
3. Open Google Search Console for `serviio.ai`.
4. Submit `https://serviio.ai/sitemap.xml`.
...
6. Check the Pages report after Google crawls the sitemap and fix any indexing or duplicate/canonical issues.

## Lead Qualification

All lead forms should preserve:
- restaurant name
...
- POS recommendation interest for restaurants without a POS
- lead source
- ideal customer profile

Priority lead definition:
- Chinese restaurant in the United States
- already uses 39 Miles, Square, Toast, Clover, MenuSifu, Chowbus, Mealkeyway, or another POS
- meaningful weekly phone order volume
- wants fewer missed calls or less manual entry

Secondary lead definition:
- no POS yet
- wants POS recommendations through `pos_recommendation_interest`
- can be nurtured or referred to a POS partner later

Operational lead scoring and follow-up steps live in `docs/seo-deploy-and-lead-runbook.md`.

## Next SEO Work

Use `docs/seo-keyword-map.md` as the working keyword matrix. The goal is to cover every meaningful combination of restaurant type, AI category, phone-order use case, POS/integration qualifier, language, and location without creating thin duplicate pages.

Content pages recently added:
- `/chinese-restaurant-pos-integration/`
- `/zh/chinese-restaurant-pos-integration/`
- `/restaurant-phone-order-ai-pos/`
- `/zh/restaurant-phone-order-ai-pos/`
...
- Get listed in restaurant technology directories.
- Publish bilingual posts in Chinese restaurant owner communities.
- Build partnership/referral pages with POS vendors and restaurant consultants.
- Ask early customers for testimonials that mention restaurant type, city, POS system, and phone-order volume.
...
- average position for POS-specific queries
- indexing status for all sitemap URLs
